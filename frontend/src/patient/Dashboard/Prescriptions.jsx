                ...
                <div className="overflow-y-auto" style={{ height: '550px' }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {prescriptionData.map((val, index) => (
                            <div key={index} className="w-full rounded-lg bg-white border border-gray-200 shadow-md">
                                <div className="flex justify-between items-center py-2 bg-gray-100 px-3">
                                    ...
                                    <button onClick={() => handleShowModal(val)} className='w-7 h-7 bg-white rounded-lg'>
            ...
            {showModal && (
                <>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center z-50">
                        <div className="bg-white rounded-lg shadow-lg w-11/12 max-w-md md:max-w-xl relative"> {/* Responsive width */}
                            ...
                            <div className="modal-body p-3 mx-3 mb-3">
                                <div className="max-w-xl mx-auto bg-bg-color rounded-lg p-3 border border-gray-200">
                                    <div className="flex justify-between items-center border-b border-dashed">
                                        <div>
                                            <div className="flex items-center space-x-2">
                                                <img src="./image/Group 1000005871.png" alt="" className="w-32 md:w-40" /> {/* Responsive image size */}
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700"> {/* Responsive grid layout */}
                                        <div>
                                            <p><span className="font-semibold">Hospital Name</span> : Medical Center</p>
                                            <p><span className="font-semibold">Patient Name</span> : Alabtrao Bhajirao</p>
                                            <p><span className="font-semibold">Gender</span> : Male</p>
                                        </div>
                                        <div className="border-l border-dashed pl-4">
                                            <p><span className="font-semibold">Prescription Date</span> : 2 Jan, 2022</p>
                                            <p><span className="font-semibold">Age</span> : 36 Years</p>
                                        </div>
                                    </div>
                                    <p className="font-semibold">Address: <span className='text-sm font-normal'>B-105 Virat Bungalows, Punagam Motavarocha Jamnagar.</span></p>
                                </div>
                                <table className="table mt-3 rounded-lg w-full"> {/* Full-width table */}
                                    ...
                                    <tbody>
                                        {prescriptionData.map((val, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>{val.dose}</td>
                                                <td>
                                                    <button className='px-2 py-1 bg-button-bg rounded-full text-green-700'>{val.duration}</button>
                                                </td>
                                                <td>
                                                    <button className='px-2 py-1 bg-btn-color rounded-full text-blue-950'>{val.whenToTake}</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                                ...
                                <div className="flex justify-between items-center">
                                    <div>
                                        <img src="./image/Rectangle 20611.png" alt="" className="w-16" /> {/* Responsive image size */}
